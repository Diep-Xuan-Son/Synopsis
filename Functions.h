...
void ConvexHull(const Mat maskMog2,Mat src,const Mat frame2, Mat &output, Mat &threshold_output)
{
	...
	int thresh = 10;
	Mat src_gray;
	cv::cvtColor( maskMog2, src_gray, cv::COLOR_BGR2GRAY );
	...
	while(true)
	{
		if (!Objects.empty()) // si hay cosas que mostrar
		{
			...
			for(int i = 0 ; i < window.size(); i++)
			{
				...
				double grosor = 1.9f;
#if EVENTOS_SEGUNDOS
				putText(frame,etiqueta, cv::Point(aux[0],aux[1]), FONT_HERSHEY_SCRIPT_SIMPLEX,	escala,	cv::Scalar(255,255,255), grosor);
#endif
...
//			http://blog.damiles.com/2010/01/segmentation-object-detection-by-color/
			...
			{
				Mat obj_hsv,img1;
				cvtColor(objeto[j],obj_hsv,cv::COLOR_RGB2HSV); // paso un frame a hsv
				Mat imgSplit[3];
#if SEG_ROJO_AZUL
